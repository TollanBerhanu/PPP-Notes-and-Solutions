import 'bootstrap';
import 'bootstrap/dist/css/bootstrap.min.css';
import './bootstrap-datetimepicker.min.css';
import * as L from 'lucid-cardano';

const vestingScript = { // This is the serealized script
    type: "PlutusV2",
    script: "",
};

const VestingDatum = L.Data.Object({ // This is the datum converted to json using a native Lucid function
    beneficiary: L.Data.String,
    deadline: L.Data.BigInt,
});

function removeChildren(elt) {
    while (elt.firstChild) {
        elt.removeChild(elt.lastChild);
    }
}

async function loadCardano() { // To connect lucid to the nami wallet
    const nami = window.cardano.nami;
    if (!nami) {
        setTimeout(loadCardano);
    } else {
        const api = await nami.enable();
        console.log('nami enabled');
        const lucid = await L.Lucid.new( // We need a blockfrost api key to get access to the blockchain network (preview testnet)
            new L.Blockfrost("https://cardano-preview.blockfrost.io/api/v0", "previewTboSqp1nB894P6wgrGA1PBv8rgUvIS5v"),
            "Preview",
        );
        console.log('lucid active');
        lucid.selectWallet(api);
        return lucid;
    }
}

async function submitCardanoTx(signedTx) { // Helper function to submit a txn (after it's signed)
    const tid = await signedTx.submit();
    console.log("Cardano tx submitted: " + tid);
    addLinkToTable("cardanoTxTable", "https://preview.cardanoscan.io/transaction/" + tid, tid);
}

async function signAndSubmitCardanoTx(tx) { // helper function to sign a transaction (using nami) and submit it
    try {
        const signedTx = await tx.sign().complete();
        await submitCardanoTx(signedTx);
    } catch (err) {
        alert(`Cardano transaction:\ninfo: ${err.info}\nmessage: ${err.message}`);
        throw (err);
    }
}

async function getCardanoPKH() { // To automatically display the pubKey hash belonging to the nami wallet
    const addr = await lucid.wallet.address(); // In this case, it's only one address because nami uses only one address
                                              // If it was eternal, it would have been a wallet with several addresses 
    const details = await L.getAddressDetails(addr);
    return details.paymentCredential.hash; // return the pubKey Hash
}

async function getStatus() {
    const pkh = await getCardanoPKH();
    const utxos = await lucid.wallet.getUtxos(); // get all the UTxOs in my wallet
    const lovelace = utxos.reduce((acc, utxo) => acc + utxo.assets.lovelace, 0n); // extract the lovelaces and add them

    const vestings = await vestingUTxOs();

    return { // These three will be the status
        cardanoPKH: pkh,
        cardanoBalance: lovelace,
        vestingUTxOs: vestings,
    };
}

// ========================== UI for table
function addCell(tr, content) {
    const td = document.createElement('td');
    tr.appendChild(td);
    td.appendChild(document.createTextNode(content));
}

function addLinkToTable(tableId, href, text) {
    const txTable = document.getElementById('cardanoTxTable');
    const tr = document.createElement('tr');
    txTable.appendChild(tr);
    const td = document.createElement('td');
    tr.appendChild(td);
    const a = document.createElement('a');
    td.appendChild(a);
    a.setAttribute('href', href);
    a.setAttribute('target', '_blank');
    a.appendChild(document.createTextNode(text));
}

function addCopyCell(row, text) {
    const td = document.createElement("td");
    row.appendChild(td);
    const span = document.createElement("span");
    td.appendChild(span);
    const uid = String(Math.random()).slice(2);
    span.setAttribute("id", uid);
    span.appendChild(document.createTextNode(text));
    const button = document.createElement("button");
    td.appendChild(button);
    button.setAttribute("type", "button");
    button.classList.add("btn");
    button.classList.add("btn-outline-primary");
    button.classList.add("btn-sm");
    button.addEventListener("click", () => onCopy(uid));
}

// ===================== Fill table with current status
async function setStatus() {
    const status = await getStatus();

    const cardanoPKH = document.getElementById('cardanoPKH');
    removeChildren(cardanoPKH);
    cardanoPKH.appendChild(document.createTextNode(status.cardanoPKH));

    const cardanoBalance = document.getElementById('cardanoBalance');
    const ada = Number(status.cardanoBalance) / 1000000;
    removeChildren(cardanoBalance);
    cardanoBalance.appendChild(document.createTextNode(ada));

    const vestingUTxOsTable = document.getElementById('vestingUTxOsTable');
    removeChildren(vestingUTxOsTable);
    for (const x of status.vestingUTxOs) {
        const tr = document.createElement('tr');
        vestingUTxOsTable.appendChild(tr);

        addCopyCell(tr, x.utxo.txHash + '#' + x.utxo.outputIndex);
        addCopyCell(tr, x.datum.beneficiary);
        addCell(tr, x.utxo.assets.lovelace);
        addCell(tr, new Date(Number(x.datum.deadline)));
    }
}

// Quering the blockchain for all the vesting UTxOs sitting at the vesting address (address of the script) 
async function vestingUTxOs() {
    const utxos = await lucid.utxosAt(vestingAddress); // grab all UTxOs at the vestingScript's Address
    const res = [];
    for (const utxo of utxos) {
        const datum = utxo.datum;
        if (datum) {
            try {
                const d = L.Data.from(datum, VestingDatum); // fetch the datum and convert it in the form of VestingDatum schema 
                res.push({  // we defined at the top and add it to the list
                    utxo: utxo, // the UTxO it self
                    datum: d // the converted datum
                });
            } catch (err) { // If the datum doesn't exist or has a wrong type, it will return an exception
            }
        }
    }
    return res;
}

//  To claim a specific vesting UTxO
async function findUTxO(ref) { // ref is the TxOutRef of the UTxO i want to claim
    const chunks = ref.split('#'); // It is a string with a hash separating it from the index
    const tid = chunks[0]; // transaction id
    const ix = parseInt(chunks[1]); // output index
    const utxos = await vestingUTxOs();
    for (const utxo of utxos) { // loop over all vesting UTxOs and check which of those has the correct reference
        if (utxo.utxo.txHash == tid && utxo.utxo.outputIndex == ix) {
            return utxo; // return the UTxO
        }
    }
    return null; 
}

// handles the onclick of 'Vest' button
async function onVest() {
    // extract the necessary input fields
    const beneficiaryText = document.getElementById('vestBeneficiaryText');
    const beneficiary = beneficiaryText.value;
    const amountText = document.getElementById('vestAmountText');
    const amount = BigInt(parseInt(vestAmountText.value));
    const deadlineText = document.getElementById('vestDeadlineText');
    const deadline = BigInt(Date.parse(deadlineText.value));

    const d = { // construct the datum
        beneficiary: beneficiary,
        deadline: deadline,
    };
    const datum = L.Data.to(d, VestingDatum); // convert the object into json (opposite of Data.from)
    const tx = await lucid // assemble a new txn
        .newTx()
        .payToContract(vestingAddress, { inline: datum }, { lovelace: amount }) // txn has one o/p with the vesting script address
        .complete();                                                            // the lovelace amount and the inline datum 
    signAndSubmitCardanoTx(tx);
// The input of the txn and the change output is taken care of by lucid automatically. Lucid will look into my wallet to find the 
// appropriate i/ps (coin selection) and add change outputs when necessary. Lucid will sign this txn automatically.

    beneficiaryText.value = "";
    amountText.value = "";
    deadlineText.value = "";
}

// onclick handler fot the 'Claim' button
async function onClaim() {
    const pkh = await getCardanoPKH(); // lookup my own pubKey Hash

    const referenceText = document.getElementById('claimReferenceText'); // get the reference from the input field
    const reference = referenceText.value;

    const utxo = await findUTxO(reference); // find hte UTxO with the reference
    if (utxo) {
        const tx = await lucid // construct the txn
            .newTx()
            .collectFrom([utxo.utxo], L.Data.to(new L.Constr(0, []))) // opposite of payToContract
                // we specify which UtxO we want to consume, and the redeemer (unit). We don't need to specify the datum because it's inline datum
            .attachSpendingValidator(vestingScript) // specify the actual vesting script
            .addSignerKey(pkh) // We have to explicitly sign the txn because the smart contract is expecting it
            .validFrom(Number(utxo.datum.deadline)) // explicitly define a vaidity interval for the txn (default interval is unbounded)
                // from Genesis block to eternity. Plutus validation will be unable to check if current time is after deadline or not.
             .complete(); 
        signAndSubmitCardanoTx(tx);
    } else {
        console.log("UTxO not found");
    }

    referenceText.value = "";
}

function onCopy(elt) {
    navigator.clipboard.writeText(document.getElementById(elt).firstChild.textContent);
}

// Get the address of the vesting script 
window.L = L;
window.lucid = await loadCardano();
const vestingAddress = lucid.utils.validatorToAddress(vestingScript);

$(function () {
    $(".dtp").datetimepicker({
        minuteStep: 1,
        autoclose: true,
        format: 'yyyy-mm-dd hh:ii'
    });
});

// update the status every 5 seconds
setStatus();
setInterval(setStatus, 5000);

document.getElementById("vestButton").addEventListener("click", onVest);
document.getElementById("claimButton").addEventListener("click", onClaim);
document.getElementById('cardanoPKHButton').addEventListener("click", () => onCopy("cardanoPKH"));